...
function SidebarContent() {
  ...
  const memoizedSideBar = useCallback(
    ...
    [mainItems, pathname, searchParams] // Dependency array
  );
  const memoizedBottomItens = useCallback(() => {
    return bottomItems.map(item => {
      const isActive = pathname === item.href;
      const IconComponent = item.icon;

      // Skip rendering if item has role restriction and user doesn't have the required role
      if (item.roleRestriction && (!user?.role || !item.roleRestriction.includes(user.role))) {
        return null;
      }
  ...
  }, [bottomItems, pathname, user?.role]);
  return (
    <div className="flex h-screen w-64 flex-col bg-white dark:bg-black py-6">
      ...
      <div className="flex-1 overflow-y-auto">
        <nav className="space-y-1 px-4">{memoizedSideBar()}</nav>
        <div className="mt-6">
          <h2
      ...
      <div className="mt-auto flex flex-col px-4 py-4">
        <nav className="space-y-1 w-full">{memoizedBottomItens()}</nav>
      </div>
    </div>
  );
}
