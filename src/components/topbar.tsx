...
export function Topbar({ title = "Dashboard", breadcrumbs = [], user, onNewTicketClick }: TopbarProps) {
    ...
    const { data: notifications = [], isLoading: isLoadingNotifications } = useQuery<Activity[], Error>({
                        ...
                        <AnimatePresence>
                            {isNotificationsOpen && (
                                ...
                                >
                                    ...
                                    <ScrollArea className="h-72"> {/* Fixed height and scroll */}
                                        <div className="p-4 space-y-4">
                                            {isLoadingNotifications ? (
                                                ...
                                                notifications.map((notification) => {
                                                    ...
                                                    const isUserCreator = isTicketCreation && (notification.creator_user_id || notification.creator_user_email);

                                                    const avatarName = isUserCreator
                                                        ? notification.creator_user_email || `user-${notification.creator_user_id}` || 'unknown-user'
                                                        : notification.agent?.email || notification.agent?.name || `agent-${notification.agent_id}` || 'system';
                                                            ...
                                                            <div className="text-sm flex-1">
                                                                <p>
                                                                    ...
                                                                    {isTicketCreation && notification.source_id && (
                                                                        <span className="text-primary ml-1">#{notification.source_id}</span>
                                                                    )}
                                                                </p>
                                                    ...
                                                    return isTicketCreation && notification.source_id ? (
                                                        ...
                                                        <div key={notification.id}> {/* Non-clickable div for other notifications */}
                                                            {notificationContent}
                                                        </div>
                                                    );
                                                })
