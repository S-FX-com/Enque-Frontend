...
export function AgentProfileForm({ agent }: AgentProfileFormProps) {
    ...
    const handleSave = () => {
        ...
        if (editedPhoneNumber !== (agent.phone_number || '')) payload.phone_number = editedPhoneNumber || null;
        if (editedRole !== agent.role) payload.role = editedRole as 'admin' | 'agent' | 'manager';
        if (editedSignature !== (agent.email_signature || '')) payload.email_signature = editedSignature || null;
    ...
    return (
        <Card className="bg-white dark:bg-black"> {/* Added background class */}
            <CardContent className="pt-4">
                ...
                <section className="mb-6">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        {isAdmin && (
                            !isEditing ? (
                                <Button variant="outline" size="sm" onClick={handleEditToggle} disabled={updateProfileMutation.isPending}>Edit</Button>
                            ) : (
                    ...
                    {!isEditing ? (
                        ...
                        <div className="space-y-2">
                            <EditRow label="Name" id="name" value={editedName} onChange={(e) => setEditedName(e.target.value)} disabled={updateProfileMutation.isPending} />
                            <EditRow label="Email" id="email" value={editedEmail} onChange={(e) => setEditedEmail(e.target.value)} disabled={updateProfileMutation.isPending} />
                            <EditRow label="Job Title" id="jobTitle" value={editedJobTitle} onChange={(e) => setEditedJobTitle(e.target.value)} disabled={updateProfileMutation.isPending} />
                            <EditRow label="Phone Number" id="phoneNumber" value={editedPhoneNumber} onChange={(e) => setEditedPhoneNumber(e.target.value)} disabled={updateProfileMutation.isPending} />
                        </div>
                    )}
                </section>
                ...
                <section className="mb-6">
                    ...
                    <div className="flex items-center gap-4">
                        <Avatar className="h-12 w-12 overflow-hidden border"> {/* Added border */}
                            <BoringAvatar
                                size={48}
                    ...
                    {!isEditing ? (
                        ...
                        <div className="grid grid-cols-3 items-center gap-4 py-1">
                            ...
                            <Select
                                ...
                                onValueChange={(value: string) => setEditedRole(value as 'admin' | 'agent' | 'manager')}
