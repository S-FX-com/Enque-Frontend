...
export function SubdomainForm() {
  ...
  useEffect(() => {
    // Detectar el dominio actual cuando el componente se monta
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        const targetHostname = `${trimmedSubdomain}${domainSuffix}`;
        ...
        setSubdomainError(
          `Workspace "${trimmedSubdomain}" not found. Please check the subdomain and try again.`
        );
      } else {
        console.error(`API error checking workspace: ${response.status} ${response.statusText}`);
        setSubdomainError(
          'An error occurred while checking the workspace. Please try again later.'
        );
      }
    } catch (error) {
      console.error('Network or fetch error checking workspace:', error);
      setSubdomainError(
        'Could not connect to the server to check the workspace. Please check your network connection.'
      );
    }
