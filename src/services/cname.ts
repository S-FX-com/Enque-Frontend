...
export const checkDomainAvailability = async (
  ...
  try {
    ...
    const response = await fetchAPI.POST<{ is_available: boolean; message?: string }>(url, {
      domain,
    });

    if (!response || !response.data) {
      console.error('Failed to check domain availability or data is missing');
...
/**
 * Gets the SSL certificate status for a domain.
 * @param workspaceId The ID of the workspace.
 * @returns A promise that resolves to a status object.
 */
export const getSslStatus = async (
  ...
  try {
    ...
    const response = await fetchAPI.GET<{ status: string; expires_at?: string; message?: string }>(
      url
    );

    if (!response || !response.data) {
      console.error('Failed to get SSL status or data is missing');
      throw new Error('Failed to get SSL status');
    }
