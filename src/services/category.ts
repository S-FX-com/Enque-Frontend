 ...
 */
export async function getCategories(params?: { skip?: number; limit?: number }): Promise<ICategory[]> {
  try {
 ...
 */
export async function createCategory(categoryData: CategoryCreatePayload): Promise<BaseResponse<ICategory>> {
  try {
    const url = `${API_BASE_URL}/v1/categories/`;
    // Use fetchAPI.POST, assuming it handles the request body correctly
    const response = await fetchAPI.POST<ICategory>(url, categoryData as unknown as Record<string, unknown>);
    ...
    return { success: false, message: error instanceof Error ? error.message : 'Failed to create category', data: undefined };
  }
}
 ...
 */
export async function deleteCategory(categoryId: number | string): Promise<BaseResponse<unknown>> { // Use unknown instead of any
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : 'Failed to delete category', data: undefined };
  }
}
