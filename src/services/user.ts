...
export async function createUser(userData: {
  ...
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : 'Failed to create user', data: undefined };
  }
}
...
export async function updateUser(
  ...
  try {
    ...
    const response = await fetchAPI.PUT<IUser>(url, updateData as unknown as Record<string, unknown>);
    return response;
  } catch (error) {
    console.error(`Error updating user ${userId} (catch block):`, error);
    return { success: false, message: error instanceof Error ? error.message : 'Failed to update user', data: undefined };
  }
}
 ...
 */
export async function getUnassignedUsers(params?: { skip?: number; limit?: number }): Promise<IUser[]> { // Using IUser for now
  try {
