...
export async function createCompany(companyData: CompanyCreate): Promise<BaseResponse<ICompany>> {
  try {
    ...
    const response = await fetchAPI.POST<ICompany>(url, companyData as unknown as Record<string, unknown>);
    return response;
  } catch (error) {
    console.error("Error creating company (catch block):", error);
    return { success: false, message: error instanceof Error ? error.message : 'Failed to create company', data: undefined };
  }
}
 ...
 */
export async function getCompanies(params?: { skip?: number; limit?: number }): Promise<ICompany[]> {
  try {
 ...
 */
export async function getCompanyUsers(companyId: number | string, params?: { skip?: number; limit?: number }): Promise<IUser[]> {
  try {
    ...
    if (response.success && response.data) {
      ...
      console.error(`Error fetching users for company ${companyId}:`, response?.message || "Unknown API error");
      return [];
    }
  } catch (error) {
...
export async function deleteCompany(companyId: number | string): Promise<BaseResponse<ICompany>> {
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : 'Failed to delete company', data: undefined };
  }
}
...
export async function updateCompany(
  ...
  try {
    ...
    const response = await fetchAPI.PUT<ICompany>(url, updateData as unknown as Record<string, unknown>);
    return response;
  } catch (error) {
    console.error(`Error updating company ${companyId} (catch block):`, error);
    return { success: false, message: error instanceof Error ? error.message : 'Failed to update company', data: undefined };
  }
}
