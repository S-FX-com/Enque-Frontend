 ...
 */
export async function getReportSummary(params?: { start_date?: string; end_date?: string }): Promise<ReportSummary | null> {
  try {
 ...
 */
export async function getCreatedByHourReport(params?: { start_date?: string; end_date?: string }): Promise<TimeSeriesDataPoint[] | null> {
  try {
    ...
    if (response.success && response.data) {
      ...
      console.error("Error fetching created_by_hour report:", response?.message || "Unknown API error");
      return null;
    }
  } catch (error) {
 ...
 * @returns A promise that resolves to an array of TimeSeriesDataPoint or null on failure.
 */
export async function getCreatedByDayReport(params?: { start_date?: string; end_date?: string }): Promise<TimeSeriesDataPoint[] | null> {
  try {
    ...
    if (response.success && response.data) {
      ...
      console.error("Error fetching created_by_day report:", response?.message || "Unknown API error");
      return null;
    }
  } catch (error) {
