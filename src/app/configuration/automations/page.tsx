...
const getAutomations = async (workspaceId: string) => {
  // Fetch automations from your API
  return [
    {
      id: '1',
      name: 'Weekly Customer Satisfaction Survey',
      description: 'Send CSAT survey to customers with closed tickets',
      is_enabled: true,
      type: 'scheduled',
      schedule: {
        frequency: 'weekly',
      ...
      template: {
        ...
        content:
          '<p>Dear [Customer Name],</p><p>We hope you were satisfied with our service. Please take a moment to rate your experience.</p>',
      },
      filters: {
        ticket_status: 'closed',
        closed_within_days: 7,
      },
    },
    {
      id: '2',
      name: 'Monthly Performance Report',
      description: 'Send monthly performance metrics to team leads',
      is_enabled: false,
      type: 'scheduled',
      schedule: {
        frequency: 'monthly',
      ...
      template: {
        ...
        content:
          "<p>Hello [Agent Name],</p><p>Here is your team's performance report for the month of [Month].</p>",
      },
      filters: {
        recipients: ['team_leads', 'admins'],
      },
    },
  ];
};

const createAutomation = async (workspaceId: string, automation: any) => {
  // Create a new automation via API
  return { ...automation, id: Date.now().toString() };
};

const updateAutomation = async (workspaceId: string, automation: any) => {
  // Update an automation via API
  return automation;
};

const deleteAutomation = async (workspaceId: string, id: string) => {
  // Delete an automation via API
  return { success: true };
};

const toggleAutomation = async (workspaceId: string, id: string, enabled: boolean) => {
  // Toggle an automation via API
