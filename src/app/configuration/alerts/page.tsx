...
const getAlertSettings = async (workspaceId: string) => {
  // Fetch alert settings from your API
  return {
    system_alerts: {
      m365_connector_disconnected: {
        is_enabled: true,
        display_type: 'banner', // banner, modal, notification
        message:
          'Microsoft 365 connector is disconnected. Please reconnect it to continue receiving emails.',
        severity: 'critical', // critical, warning, info
        actions: [
          {
            label: 'Reconnect',
            action_type: 'link',
            action_value: '/settings/integrations/m365',
          },
        ],
      },
      database_connection_error: {
        is_enabled: true,
        display_type: 'banner',
        message: 'Database connection error. Some features may not work properly.',
        severity: 'critical',
        actions: [
          {
            label: 'View Status',
            action_type: 'link',
            action_value: '/settings/system/status',
          },
        ],
      },
      license_expiring: {
        ...
        message:
          'Your license will expire in [days_remaining] days. Please renew to avoid service interruption.',
        severity: 'warning',
        actions: [
          {
            label: 'Renew License',
            action_type: 'link',
            action_value: '/settings/billing',
          },
        ],
      },
    },
    admin_notifications: {
      new_agent_joined: {
        ...
        template: '<p>A new agent [Agent Name] has joined your workspace.</p>',
      },
      high_ticket_volume: {
        is_enabled: true,
        channels: ['email', 'in_app'],
        template: '<p>Unusually high ticket volume detected in the last hour.</p>',
        threshold: 50,
      },
      system_update: {
        is_enabled: true,
        channels: ['email', 'in_app'],
        template: '<p>System update scheduled for [Update Date].</p>',
      },
    },
  };
};

const updateAlertSetting = async (workspaceId: string, path: string, value: any) => {
  // Update alert setting via API
  return { success: true };
};

const toggleAlertSetting = async (workspaceId: string, path: string, enabled: boolean) => {
  // Toggle alert setting via API
  return { success: true };
};

const testM365Connection = async (workspaceId: string) => {
  // Test M365 connection via API
...
export default function AlertsConfigPage() {
  ...
  const handleEditAlert = (alertKey: string, alert: any) => {
    setCurrentAlert({ key: alertKey, ...alert });
  ...
  const handleEditNotification = (key: string, notification: any) => {
    setCurrentAlert({ key, ...notification });
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogContent className="sm:max-w-2xl">
              <DialogHeader>
                <DialogTitle>
                  {isEditingTemplate
                    ? `Edit ${currentTemplateTitle} Template`
                    : currentAlert?.key === 'new_agent_joined' ||
                        currentAlert?.key === 'high_ticket_volume' ||
                        currentAlert?.key === 'system_update'
                      ? 'Edit Admin Notification'
                      : 'Edit System Alert'}
                </DialogTitle>
                <DialogDescription>
                  {isEditingTemplate
                    ? `Customize the template for ${currentTemplateTitle.toLowerCase()} notifications.`
                    : currentAlert?.key === 'new_agent_joined' ||
                        currentAlert?.key === 'high_ticket_volume' ||
                        currentAlert?.key === 'system_update'
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        Available placeholders: [Agent Name], [Ticket ID], [Update Date]
                      </p>
                    </div>
                  </div>
                ) : currentAlert?.key === 'new_agent_joined' ||
