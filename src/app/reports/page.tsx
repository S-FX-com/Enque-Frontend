...
import { getReportSummary, getCreatedByHourReport, getCreatedByDayReport } from '@/services/reports';
...
const renderBarChart = (title: string, data: ChartDataPoint[] | null | undefined, dataKey: string, nameKey: string, isLoading: boolean) => (
    <Card>
        <CardHeader>
   ...
   const { data: createdByDayData, isLoading: isLoadingCreatedByDay } = useQuery({
    ...
    const statusChartData: ChartDataPoint[] = useMemo(() => (
        summaryData?.status_counts
            ? Object.entries(summaryData.status_counts).map(([status, count]) => ({ name: status, count: count ?? 0 }))
            : []
    ), [summaryData]);

    const priorityChartData: ChartDataPoint[] = useMemo(() => (
        summaryData?.priority_counts
            ? Object.entries(summaryData.priority_counts).map(([priority, count]) => ({ name: priority, count: count ?? 0 }))
            : []
    ), [summaryData]);
     ...
     const dayChartData: ChartDataPoint[] = useMemo(() => (
        createdByDayData
            ? createdByDayData.map((item: TimeSeriesDataPoint) => ({ name: item.time_unit, count: item.count }))
            : []
    ), [createdByDayData]);

    const renderSummaryCard = (title: string, value: string | number | null | undefined, isLoading: boolean) => (
        <Card>
    ...
    return (
        <div className="container mx-auto px-4 md:px-6 py-6 space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
                ...
                {renderSummaryCard("Average Response Time", summaryData?.average_response_time, isLoadingSummary)}
                {renderSummaryCard("Avg. First Response Time", summaryData?.avg_first_response_time, isLoadingSummary)}
            </div>
