...
export function TicketPageContent({ ticketId }: Props) {
  ...
  // ✅ FIX: Wrapped in useCallback to prevent dependency issues
  const isMailboxEmail = useCallback((email: string): boolean => {
  ...
  const { data: latestCommentData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return comments.find((c: IComment) => !c.is_private) || null;
    },
  ...
  }, [currentTicket, ticketError, latestCommentData, isMailboxEmail]);
