...
export function CategoriesClientContent() {
  ...
  const isIndeterminate =
    selectedCategoryIds.size > 0 && selectedCategoryIds.size < categories.length;
  const headerCheckboxState = isAllSelected ? true : isIndeterminate ? 'indeterminate' : false;

  const deleteCategoriesMutation = useMutation({
    mutationFn: async (categoryIds: number[]) => {
      const results = await Promise.allSettled(categoryIds.map(id => deleteCategory(id)));
      ...
      const nonSuccessResponses = results
        ...
        .filter(
          item =>
            ...
            !(item.result as PromiseFulfilledResult<BaseResponse<unknown>>).value.success
        );
    ...
    onSuccess: (data, variables) => {
      const allSucceeded = data.every(
        result => result.status === 'fulfilled' && (result.value as BaseResponse<unknown>).success
      );

      if (allSucceeded) {
        toast.success(
          `${variables.length} categor${variables.length === 1 ? 'y' : 'ies'} deleted successfully!`
        );
      } else {
    ...
    onError: (
      err: Error,
      categoryIdsToDelete,
      context: { previousCategories?: ICategory[] } | undefined
    ) => {
