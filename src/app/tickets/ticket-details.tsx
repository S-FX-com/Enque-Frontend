...
export function TicketDetail({ ticket, onClose, onTicketUpdate }: Props) {
    ...
    const updateFieldMutation = useMutation<
        ITicket, 
        Error, 
        { field: 'priority' | 'assignee_id' | 'team_id' | 'category_id'; value: string | null; originalFieldValue: TicketPriority | number | null }, // Type of variables passed to mutationFn
    ...
    >({
        ...
        onSuccess: async (data, variables) => {
            ...
            if (currentUserId) {
                if (variables.field === 'assignee_id') {
                    ...
                    if (previousAssigneeId === currentUserId || newAssigneeId === currentUserId) {
                        console.log(`Assignee changed involving current user ${currentUserId}. Invalidating 'My Tickets' query.`);
            ...
            } else { // If currentUserId is not available
                 queryClient.invalidateQueries({ queryKey: ['tickets'] });
                 if (variables.field === 'team_id') {
    ...
    const handleUpdateField = (field: 'priority' | 'assignee_id' | 'team_id' | 'category_id', value: string | null) => {
        if (!ticket) return;
                         ...
                         <div className="flex-1 space-y-4 overflow-y-auto">
                            ...
                            <div>
                             ...
                             <div>
                                ...
                                >
                                    <SelectTrigger className="h-8 text-xs focus:ring-0 focus:ring-offset-0">
                                        <SelectValue placeholder={isLoadingTeams ? "Loading..." : (isUpdatingTeam ? "Updating..." : "Select team")} />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="null">Unassigned</SelectItem>
                                        {teams.map((team: Team) => ( 
                             ...
                             <div>
                                ...
                                >
                                    <SelectTrigger className="h-8 text-xs focus:ring-0 focus:ring-offset-0">
                                        <SelectValue placeholder={isLoadingCategories ? "Loading..." : (isUpdatingCategory ? "Updating..." : "Select category")} />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="null">Unassigned</SelectItem>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <BoringAvatar
                                        size={24} 
                                        name={ticket.user?.email || ticket.user?.name || `user-${ticket.user?.id}` || 'unknown-user'} 
                                        variant="beam"
                                        colors={avatarColors}
                                    />
