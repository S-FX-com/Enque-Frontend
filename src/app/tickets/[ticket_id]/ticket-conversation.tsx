...
function OptimizedMessageItem({ content, isInitial = false, ticket }: OptimizedMessageItemProps) {
  ...
  const senderInfo = React.useMemo(() => {
    ...
    if (metadataMatch && metadataMatch[1] && metadataMatch[2]) {
      return {
        ...
        company_logo_url: content.sender.company_logo_url,
      }
    }
  ...
  const avatarColors =
    isInitial || senderInfo.isUserReply || senderInfo.type === "user" ? userAvatarColors : agentAvatarColors
  ...
  const avatarSrc =
    senderInfo.type === "agent" ? senderInfo.avatar_url : senderInfo.avatar_url || senderInfo.company_logo_url
    ...
    <div className="flex-shrink-0">
      {avatarSrc ? (
        <Avatar className="w-10 h-10">
          <AvatarImage src={avatarSrc || "/placeholder.svg"} alt={senderInfo.name || 'Avatar'} />
          <AvatarFallback>
...
export function TicketConversation({
  ...
  const conversationItems = React.useMemo(() => {
    ...
    if (initialMessageContent && ticket.created_at && !initialMessageContent.startsWith("[MIGRATED_TO_S3]")) {
  ...
  useEffect(() => {
    ...
    const initialContent = signatureToUse ? `<p style="margin-block: 10px !important"><p>${signatureToUse}` : ``
  ...
  const handleCannedReplySelect = (cannedReply: CannedReply) => {
    ...
    if (isPrivateNote) {
      ...
      const fullContent = signatureToUse ? `${greeting}${content}<p><br></p>${signatureToUse}` : `${greeting}${content}`
                      ...
                      <div className="p-2">
                        {isLoadingCannedReplies ? (
                          ...
                          <div className="text-center text-muted-foreground py-4 text-sm">
                            {cannedSearchTerm ? "No templates match your search" : "No templates available"}
                          </div>
                        ) : (
          ...
          <div className="space-y-4">
            {isHtmlContentError && isCommentsError && !isLoadingHtmlContent && !isLoadingComments && (
              <div className="text-center text-red-500 py-4">
                Failed to load conversation: {htmlContentError?.message || commentsError?.message || "Unknown error"}
              </div>
            )}
            ...
            !(isHtmlContentError && isCommentsError) ? (
              <div className="text-center text-muted-foreground py-10">No conversation history found.</div>
            ) : (
              <>
