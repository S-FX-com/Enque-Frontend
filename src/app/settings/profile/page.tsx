...
const EditRow: React.FC<{ label: string; id: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; disabled?: boolean }> = ({ label, id, value, onChange, disabled }) => (
...
export default function ProfileSettingsPage() {
    ...
    const displayData = {
        ...
        updatedAt: agentProfileData?.updated_at ? new Date(agentProfileData.updated_at).toLocaleString() : "...",
        createdAt: agentProfileData?.created_at ? new Date(agentProfileData.created_at).toLocaleString() : "...",
    };
    ...
    const handleSave = () => {
        ...
        if (editedJobTitle !== (agentProfileData.job_title || '')) payload.job_title = editedJobTitle || null;
        if (editedPhoneNumber !== (agentProfileData.phone_number || '')) payload.phone_number = editedPhoneNumber || null;
        if (editedRole !== (agentProfileData.role || 'agent')) payload.role = editedRole as 'admin' | 'agent' | 'manager';
        if (editedSignature !== (agentProfileData.email_signature || '')) payload.email_signature = editedSignature || null;
                    ...
                    <section className="mb-6">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            {(currentUserRole === 'admin' || agentProfileData?.id === user?.id) && (
                                !isEditing ? (
                                    <Button variant="outline" size="sm" onClick={handleEditToggle} disabled={isLoadingProfile || isLoadingAuthUser}>Edit</Button>
                                ) : (
                                    <div className="flex gap-2">
                                        <Button variant="outline" size="sm" onClick={handleEditToggle} disabled={updateProfileMutation.isPending}>Cancel</Button>
                        ...
                        {isLoadingProfile || isLoadingAuthUser ? (
                            ...
                            <div className="space-y-2">
                                <EditRow label="Name" id="name" value={editedName} onChange={(e) => setEditedName(e.target.value)} disabled={updateProfileMutation.isPending || currentUserRole !== 'admin'} />
                                <EditRow label="Email" id="email" value={editedEmail} onChange={(e) => setEditedEmail(e.target.value)} disabled={updateProfileMutation.isPending || currentUserRole !== 'admin'} />
                                <EditRow label="Job Title" id="jobTitle" value={editedJobTitle} onChange={(e) => setEditedJobTitle(e.target.value)} disabled={updateProfileMutation.isPending || currentUserRole !== 'admin'} />
                                <EditRow label="Phone Number" id="phoneNumber" value={editedPhoneNumber} onChange={(e) => setEditedPhoneNumber(e.target.value)} disabled={updateProfileMutation.isPending || currentUserRole !== 'admin'} />
                            </div>
                        )}
                    </section>
                    ...
                    <section className="mb-6">
                        ...
                        {!isEditing ? (
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: displayData.email_signature || '<p class="text-muted-foreground">No signature set.</p>' }}
                            />
                        ) : (
                            currentUserRole === 'admin' ? (
                                <RichTextEditor
