...
export default function ForgotPasswordPage() {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-4 bg-[#F4F7FE]">
      {' '}
      {/* Changed div to main and updated bg */}
      {/* Logo moved inside the Card */}
      <Card className="w-full max-w-sm bg-white p-8 rounded-lg shadow-md">
        {' '}
        {/* Matched signin Card style */}
        <div className="flex justify-center mb-6">
          {' '}
          {/* Logo container from signin */}
          <Image src="/enque.png" alt="Enque Logo" width={120} height={40} priority />
        </div>
        <CardHeader className="p-0 mb-4">
          {' '}
          {/* Adjusted padding and margin for header */}
          <CardDescription className="text-center">
            {' '}
            {/* Removed pt-2 as title is gone and logo provides spacing */}
        ...
        <CardContent>
          {message && (
            <div
              className={`mb-4 p-3 rounded-md text-sm ${mutation.isError ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}
            >
              {message}
            </div>
          )}
