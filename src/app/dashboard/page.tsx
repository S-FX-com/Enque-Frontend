...
export default function DashboardPage() {
  ...
  const { data: user, isLoading: isLoadingUser, isError: isUserError } = useQuery<UserSession | null, Error>({
    queryKey: ['currentUser'],
    queryFn: getCurrentUser,
    staleTime: Infinity,
  });

  const { data: assignedTickets = [], isLoading: isLoadingAssignedTickets } = useQuery<Task[], Error>({
  ...
  const isLoading = isLoadingUser || (!!user?.id && (isLoadingAssignedTickets || isLoadingUserTeams || isLoadingTeamTasks));
        ...
        <div className="flex items-center justify-center h-full">
          <div className="text-center">
            ...
            <p className="text-slate-500 mt-2">
              {isUserError ? "Failed to load user information." : "User not found."} Please try logging in again.
            </p>
          </div>
        </div>
      );
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="bg-white dark:bg-black rounded-lg p-6 flex flex-col items-center">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-6 text-center capitalize">{user.role}</p>
          ...
          <table className="w-full">
            ...
            <tbody>
              {assignedTickets.length === 0 ? (
                 <tr className="border-0"><td colSpan={3} className="text-center py-4 text-muted-foreground">No assigned tickets</td></tr>
              ) : (
      ...
      <div className="bg-white dark:bg-black rounded-lg p-6 flex flex-col">
        ...
        <div className="space-y-6 overflow-y-auto h-72 flex-grow pr-2">
          {teamsStats.length === 0 ? (
            ...
            teamsStats.map((team) => (
              <div key={team.id} className="border-b border-slate-100 dark:border-slate-800 pb-4 last:border-0 last:pb-0">
