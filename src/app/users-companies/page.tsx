...
export default function UsersCompaniesPage() {
  ...
  const [assigningUserCompany, setAssigningUserCompany] = useState<{ userId: number | null, companyId: string | null }>({ userId: null, companyId: null });
  ...
  const {
    ...
    queryFn: () => selectedCompanyId ? getCompanyUsers(selectedCompanyId, { limit: 1000 }) : Promise.resolve([]),
  ...
  const assignCompanyMutation = useMutation({
    ...
    onSuccess: (updatedUser, variables) => {
      ...
      if (variables.companyId) {
        queryClient.invalidateQueries({ queryKey: ['companyUsers', variables.companyId.toString()] });
      }
  ...
  return (
    <div className="flex flex-col h-full p-4 md:p-6 gap-4">
      ...
      <div className="flex items-center justify-start flex-wrap gap-2">
        <Button
          ...
          className={cn(
            ...
            currentView === 'unassigned_users' && "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-50 border-slate-300 dark:border-slate-700"
          )}
          onClick={() => { setCurrentView('unassigned_users'); setSelectedCompanyId(null); }}
        >
          Unassigned Users
        </Button>
        <Button onClick={() => setIsNewCompanyModalOpen(true)}><PlusCircle className="mr-2 h-4 w-4" /> Add a Company</Button>
        <Button onClick={() => setIsNewUserModalOpen(true)}><PlusCircle className="mr-2 h-4 w-4" /> Add a User</Button>
      </div>
              ...
              <div className="space-y-1 pr-4">
                {isLoadingCompanies ? (
                  ...
                  companies.map((company) => (
                    <button
                      key={company.id}
                      onClick={() => { setSelectedCompanyId(company.id.toString()); setCurrentView('company_users'); }}
                    ...
                    >
                      <Avatar className="h-8 w-8 text-xs"><AvatarFallback><Building className="h-4 w-4 text-muted-foreground" /></AvatarFallback></Avatar>
        ...
        <Card className="md:col-span-2 lg:col-span-3 shadow-sm border-0 flex flex-col">
          {currentView === 'company_users' && selectedCompany ? (
            ...
            <div className="w-full h-full flex flex-col">
               <h2 className="text-xl font-semibold mb-4 px-6 pt-6 flex-shrink-0">Unassigned Users</h2>
                     ...
                     <TableBody>
                       {unassignedUsers.map((user) => (
                         <TableRow key={user.id}>
                           ...
                           <TableCell>
                             <Select
                               value={assigningUserCompany.userId === user.id ? assigningUserCompany.companyId ?? undefined : undefined}
                               onValueChange={(value) => {
                                 const targetCompanyId = value === 'null' ? null : parseInt(value, 10);
                                 if (targetCompanyId !== null) {
                                     setAssigningUserCompany({ userId: user.id, companyId: value });
                                     assignCompanyMutation.mutate({ userId: user.id, companyId: targetCompanyId });
                                 }
                               }}
                               disabled={isLoadingCompanies || (assignCompanyMutation.isPending && assigningUserCompany.userId === user.id)}
                             >
      ...
      {/* Render Modals */}
      <NewCompanyModal isOpen={isNewCompanyModalOpen} onClose={() => setIsNewCompanyModalOpen(false)} onSaveSuccess={handleCompanySaveSuccess} />
      <NewUserModal isOpen={isNewUserModalOpen} onClose={() => setIsNewUserModalOpen(false)} onSaveSuccess={handleUserSaveSuccess} />
    </div>
  );
}
